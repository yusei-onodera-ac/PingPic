...
with sample `prompt_pool` entries, a `daily_schedules` doc for today, and a test `groups` doc, so
`admin-panel` and `mobile` have something to render locally without manually clicking through the
Emulator UI each time.
